  ...
  : QDialog(parent), ui(new Ui::DialogMultifilePrefix)
{
  ...
  {
    // look for = in the parent directory, if there is one the data is probably in a hive structre 
    std::filesystem::path path = filename.toStdString();
    std::string hive_path;
    bool found_hive = false;
    while (path.has_parent_path() && path.parent_path().filename().string().find('=') != std::string::npos){ 
